...
func init() {
	if _, ok := os.LookupEnv("HYPERION_TEST_EVM_RPC"); !ok {
		os.Setenv("HYPERION_TEST_EVM_RPC", "http://localhost:8545")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_COVERAGE"); !ok {
		os.Setenv("HYPERION_TEST_COVERAGE", "0")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_COVERAGE_MODE"); !ok {
		os.Setenv("HYPERION_TEST_COVERAGE_MODE", "set")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_BECH32_PREFIX"); !ok {
		os.Setenv("HYPERION_TEST_BECH32_PREFIX", "helios")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_BIP44_COIN"); !ok {
		os.Setenv("HYPERION_TEST_BIP44_COIN", "60")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_BIP44_HDPATH"); !ok {
		os.Setenv("HYPERION_TEST_BIP44_HDPATH", "m/44'/60'/0'/0")
	}
	if _, ok := os.LookupEnv("HYPERION_TEST_SIGNING_ALGO"); !ok {
		os.Setenv("HYPERION_TEST_SIGNING_ALGO", "secp256k1")
	}
	...
	var (
		...
		Bech32Prefix = os.Getenv("HYPERION_TEST_BECH32_PREFIX")
		...
		Bip44CoinType, _ = strconv.Atoi(os.Getenv("HYPERION_TEST_BIP44_COIN"))
		// BIP44HDPath is the BIP44 HD path used on Ethereum.
		BIP44HDPath = os.Getenv("HYPERION_TEST_BIP44_HDPATH")
	)
...
func (a *Account) Parse() {
	if len(a.Mnemonic) > 0 {
		...
		algo, err := keyring.NewSigningAlgoFromString(os.Getenv("HYPERION_TEST_SIGNING_ALGO"), keyring.SigningAlgoList{
			hd.Secp256k1,
		})
		orPanic(err)
