...
func (l *Orchestrator) relayTokenBatch(ctx context.Context, latestEthValset *hyperiontypes.Valset) error {
	...
	batches, err := l.helios.LatestTransactionBatches(ctx, hyperionId)
	...
	for _, batch := range batches {
		...
		sigs, err := l.helios.TransactionBatchSignatures(ctx, batch.BatchNonce, gethcommon.HexToAddress(batch.TokenContract), hyperionId)
		log.Info("sigs", sigs)
		if err != nil {
			return err
		}
